...
export default function StepContentForm({
  ...
  updateStepAction: (courseId: string, currentStep: IStepClient) => Promise<void>;
  deleteStepAction: (courseId: string, moduleId: string, currentStep: IStepClient) => Promise<void>;
}) {
  ...
  return (
    <form className='flex flex-col gap-2'>
      <textarea
        ...
        onChange={(e) => setStep((prev) => ({ ...prev, content: e.target.value }))}
      ></textarea>
      <div className='flex gap-4'>
        <button
          ...
          type='button'
          onClick={() => updateStepAction(courseId, step)}
        >
          Сохранить
        </button>
        <button
          ...
          type='button'
          onClick={() => deleteStepAction(courseId, searchParams.module, step)}
        >
          Удалить шаг 🗑️
        </button>
      </div>
