...
import type { ICourse, ICourseClient, ICourseContentClient } from '@/types/Course.interface';
// Типы и интерфейсы
import type { id } from '@/types/id.type';
import type { ILessonClient, ILessonContentClient } from '@/types/Lesson.interface';
import type { IModuleClient, IModuleContentClient } from '@/types/Module.interface';
...
export async function saveAndReturnCourse(
  ...
  const newCourseClient = await mongoose.connection.transaction(async (session) => {
    const existingCourse = await Course.exists({ title, author }).session(session);
    if (existingCourse) {
      await cloudinary.uploader.destroy(courseData.imageId!);

      throw new Error('Курс с таким же названием и от того же автора уже существует');
    }
 ...
 */
export async function getCourses(opts?: { limit?: number }): Promise<ICourseClient[]> {
  await dbConnect();
 ...
 * @returns {Promise<ICourseContentClient>} - Полный курс со всем содержимым.
 */
export async function getCourseFull(courseId: id): Promise<ICourseContentClient> {
  await dbConnect();
  ...
  if (!course) throw new Error(`Курс с переданным id ${courseId} не найден в базе данных`);
...
export async function deleteCourse(courseId: id): Promise<void> {
  ...
  await mongoose.connection.transaction(async (session) => {
    const course: ICourse | null = await Course.findById(courseId).lean<ICourse>().session(session);
    ...
    try {
      ...
      console.log(error instanceof Error ? error.message : error, imageId);
    }
  });
